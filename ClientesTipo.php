...
<html lang="es">
    ...
    <br><br>
    <form class="row px-3" method="post" action="ClientesTipo.php" id="formulario" class="formulario">
        <div class="container">
            <div class = "row">
                <div class="col-md-2">
                    <label class="input-group-text" for="inputGroupSelect01">TipoCliente</label>
                    <select name = "TipoCliente" class="form-select" id="inputGroupSelect01">
                        ...
                        <div class="col-md-12">
                            <table class="table table-hover table-bordered text-center">
                                ...
                                <?php
                                    ...
                                    $sql = " select c.idCliente, c.Nombre, c.Telefono, c.Celular, c.Domicilio, c.CP, 
                                    c.Ciudad, c.Estado, c.TipoCliente, c.Correo, c.RFC, c.LimiteCredito, c.Descuento, c.Saldo
                                    FROM clientes as c WHERE c.TipoCliente = '$tipo';order by c.Nombre ASC";
                                ...
                                <tr>
                                        ...
                                        <td>
                                            <?php
                                            $query = "SELECT SUM(Saldo) AS total FROM clientes ";
    ...
    <?php
            $archivo_csv=fopen('TipoCliente.csv','w');
            if($archivo_csv){
                include 'Conexion.php';
                $tipo = $_POST['TipoCliente'];
                $R="select c.idCliente, c.Nombre, c.Telefono, c.Celular, c.Domicilio, c.CP, 
                                    c.Ciudad, c.Estado, c.TipoCliente, c.Correo, c.RFC, c.LimiteCredito, c.Saldo, c.Descuento
                                    FROM clientes as c WHERE c.TipoCliente = '$tipo' order by c.Nombre ASC;";
                $resultado= mysqli_query($enlace,$R);
            
                                fputs($archivo_csv,"idCliente, Nombre, Telefono, Celular, Domicilio, CP, Ciudad, Estado, TipoCliente, Correo, RFC, LimiteCredito, Saldo, Descuento".PHP_EOL);

                while($row = mysqli_fetch_array($resultado))
                {
                    $filacvs=$row['0'].",".$row['1'].",".$row['2'].",".$row['3'].",".$row['4'].",".$row['5'].",".$row['6'].",".$row['7'].",".$row['8'].",".$row['9'].",".$row['10'].",".$row['11'].",".$row['12'].",".$row['13'];
    ...
    ?>
                     <div class="row">
                      <div class="col-12">
                          <br>
                          <a href="TipoCliente.csv" download="TipoCliente.csv" class= "btn btn-primary btn-lg">Descargar Cliente Por Tipo CSV</a>
                     </div>
        </div>
