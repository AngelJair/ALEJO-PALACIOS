...
<html lang="es">
    ...
    <br><br><br>
    <h2>PROVEEDORES POR REPRESENTANTE</h2>
    <br><br>
    <form class="row px-3" method="post" action="ProveedoresRepre.php" id="formulario" class="formulario">
        <div class="container">
            <div class = "row">
                <div class="col-md-2">
                    <label class="input-group-text" for="inputGroupSelect01">Representante</label>
                    <select name = "Representante" class="form-select" id="inputGroupSelect01">
                        <?php
                        ...
                        <div class="col-md-12">
                            <table class="table table-hover table-bordered text-center">
                                ...
                                <?php
                                    ...
                                    $sql = " select p.idProveedor, p.Nombre, p.Representante, p.Telefono, p.Celular, p.Domicilio, p.CP, 
                                    p.Ciudad, p.Estado, p.Correo, p.RFC, p.Saldo
                                    FROM proveedores AS p WHERE p.Representante= '$repre' order by p.Nombre ASC;";
                                    $resultado = mysqli_query($enlace, $sql);
                                    while ($fila = mysqli_fetch_array($resultado)) {
                                ?>
                                ...
                                <tr>
                                        ...
                                        <td>
                                            <?php
                                            $query = "SELECT SUM(Saldo) AS total FROM proveedores";
    ...
    <?php
            $archivo_csv=fopen('ProveedorRepre.csv','w');
            if($archivo_csv){
                $repre = $_POST['Representante'];
                $R= " select p.idProveedor, p.Nombre, p.Representante, p.Telefono, p.Celular, p.Domicilio, p.CP, 
                p.Ciudad, p.Estado, p.Correo, p.RFC, p.Saldo
                FROM proveedores AS p WHERE p.Representante= '$repre' order by p.Nombre ASC;";
                $resultado= mysqli_query($enlace,$R);
            
                                fputs($archivo_csv,"idProveedor, Nombre, Representante, Telefono, Celular, Domicilio, CP, Ciudad, Estado, Correo, RFC, Saldo".PHP_EOL);

                while($row = mysqli_fetch_array($resultado))
                {
                    $filacvs=$row['0'].",".$row['1'].",".$row['2'].",".$row['3'].",".$row['4'].",".$row['5'].",".$row['6'].",".$row['7'].",".$row['8'].",".$row['9'].",".$row['10'].",".$row['11'];
    ...
    ?>
                     <div class="row">
                      <div class="col-12">
                          <br>
                          <a href="ProveedorRepre.csv" download="ProveedorRepre.csv" class= "btn btn-primary btn-lg">Descargar Provedores Por Representante CSV</a>
                     </div>
        </div>
    <br><br>
</html>