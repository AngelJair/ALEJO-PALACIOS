...
<html lang="es">
    ...
    <br>
    <h2>VENTAS VENCIDAS POR CLIENTE</h2>
    <br><br>
    <form class="row px-3" method="post" action="VentasVencidasXCte.php" id="formulario" class="formulario">
        <div class="container">
            <div class="row">
                <div class="col-md-2">
                            ...
                            <table class="table table-hover table-bordered text-center">
                                ...
                                <?php
                                    ...
                                    $sql = "select v.idVenta, c.Nombre, v.Fecha, v.FechaVencimiento, v.Condicion, v.Subtotal, v.IVA, v.Total, v.Saldo 
                                            ...
                                            where c.idCliente = '$Cte' and v.FechaVencimiento <= '$fecha_actual' and v.Saldo > 0 order by c.Nombre ASC;";
                                    $resultado = mysqli_query($enlace, $sql);
                                    while ($fila = mysqli_fetch_array($resultado)) {
                                ?>
    ...
    <?php
            $archivo_csv=fopen('VentaVencidaCliente.csv','w');
            if($archivo_csv){
                $Cte = $_POST['idCliente'];
                $fecha_actual = date("Y-m-d");
                $R = "select v.idVenta, c.Nombre, v.Fecha, v.FechaVencimiento, v.Condicion, v.Subtotal, v.IVA, v.Total, v.Saldo 
                        from ventas as v INNER JOIN clientes as c on v.idCliente = c.idCliente
                        where c.idCliente = '$Cte' and v.FechaVencimiento <= '$fecha_actual' and v.Saldo > 0 order by c.Nombre ASC;";
                $resultado= mysqli_query($enlace,$R);
            
                                fputs($archivo_csv,"idVenta, Nombre, Fecha, FechaVencimiento, Condicion, Subtotal, IVA, Total, Saldo ".PHP_EOL);

                while($row = mysqli_fetch_array($resultado))
                {
                    $filacvs=$row['0'].",".$row['1'].",".$row['2'].",".$row['3'].",".$row['4'].",".$row['5'].",".$row['6'].",".$row['7'].",".$row['8'];
    ...
    ?>
                     <div class="row">
                      <div class="col-12">
                          <br>
                          <a href="VentaVencidaCliente.csv" download="VentaVencidaCliente.csv" class= "btn btn-primary btn-lg">Descargar Ventas Vencidas Por Cliente CSV</a>
                     </div>
        </div>
    <br><br>
</html>