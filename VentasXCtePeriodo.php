...
<html lang="es">
    ...
    <br>
    <h2>VENTAS POR CLIENTE Y PERIODO</h2>
    <br><br>
    <form class="row px-3" method="post" action="VentasXCtePeriodo.php" id="formulario" class="formulario">
        <div class="container">
            <div class="row">
                <div class="col-md-2">
                            ...
                            <table class="table table-hover table-bordered text-center">
                                ...
                                <?php
                                    ...
                                    $sql = "select v.idVenta, c.Nombre, v.Fecha, v.Condicion, v.Subtotal, v.IVA, v.Total, v.Saldo 
                                            ...
                                            where c.idCliente = '$Cte' AND v.Fecha BETWEEN '$FI' AND '$FF' order by c.Nombre ASC;";
                                    $resultado = mysqli_query($enlace, $sql);
                                    while ($fila = mysqli_fetch_array($resultado)) {
                                ?>
    ...
    <?php
            $archivo_csv=fopen('VentaClientePeriodo.csv','w');
            if($archivo_csv){
                $Cte = $_POST['idCliente'];
                $FI = $_POST['FI'];
                $FF = $_POST['FF'];
                $R="select v.idVenta, c.Nombre, v.Fecha, v.Condicion, v.Subtotal, v.IVA, v.Total, v.Saldo 
                     from ventas as v INNER JOIN clientes as c on v.idCliente = c.idCliente
                     where c.idCliente = '$Cte' AND v.Fecha BETWEEN '$FI' AND '$FF' order by c.Nombre ASC;";
    ...
    ?>
                     <div class="row">
                      <div class="col-12">
                          <br>
                          <a href="VentaClientePeriodo.csv" download="VentaClientePeriodo.csv" class= "btn btn-primary btn-lg">Descargar Ventas Por Cliente y Periodo CSV</a>
                     </div>
        </div>
    <br><br>
</html>